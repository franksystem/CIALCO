...
public class CommonController {
	...
	public void init() {
		...
		//Fjbl 28-03-2016
		List<UsuarioPerfilDTO> usuarioPerfilDTO = this.sessionUser.getUsuarioDTO().getUsuarioPerfilDTOs();
		for(Iterator<UsuarioPerfilDTO> it = usuarioPerfilDTO.iterator(); it.hasNext();){
			this.usuarioPerfilDTO = it.next();
		}
		
		if (this.usuarioPerfilDTO.getPerfilDTO().getMenuDTOs()!= null) {

			for (MenuDTO menuDTO : this.usuarioPerfilDTO.getPerfilDTO().getMenuDTOs()) {
				if (menuDTO.getIdMenuPadre() == null) {
					DefaultSubMenu mainMenu = new DefaultSubMenu(menuDTO.getNombre());
		...
		if (menuHijaDTOs != null && !menuHijaDTOs.isEmpty()) {
			for (MenuDTO menuDTO : menuHijaDTOs) {
				...
				if (menuDTO.getIcono() == null || menuDTO.getIcono().isEmpty()) {
					DefaultSubMenu menuPadre = new DefaultSubMenu(menuDTO.getNombre());
					menuPadre.setElements(loadMenuHijas(menuDTO.getMenuHijaDTOs()));
